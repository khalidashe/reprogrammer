...
// preserveLocalPrivateFields: the read-side guard for the not-consented case.
// A remote behavior whose journal was purged keeps the local journal.
{
  const remote = { id: 'b1', title: 'Read', journal: undefined, level: 3 } as Record<string, unknown>;
  const local = { id: 'b1', title: 'Read', journal: 'my private note', level: 2 } as Record<string, unknown>;
  const out = preserveLocalPrivateFields('behaviors', remote, local);
  expect(out.journal === 'my private note', 'local journal is preserved over a purged remote');
  expect(out.level === 3, 'non-private fields still come from the merged remote row');
}
// A remote row we have never seen locally surfaces no private writing.
{
  const remote = { id: 'b2', title: 'New', journal: 'leaked?' } as Record<string, unknown>;
  const out = preserveLocalPrivateFields('behaviors', remote, undefined);
  expect(out.journal === undefined, 'unseen remote row exposes no private field without consent');
  expect(out.title === 'New', 'non-private fields on an unseen row are kept');
}
// Entities with no private fields pass through untouched.
{
  const remote = { id: 'r1', status: 'scheduled' } as Record<string, unknown>;
  expect(
    preserveLocalPrivateFields('reminderAttempts', remote, undefined) === remote,
    'non-private entity is returned as-is',
  );
}
